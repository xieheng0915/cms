<?php session_start();
	...
	if(isset($_POST['submit_post'])){
		...
		if($_FILES['image']['name'] != ''){
            ...
            if($image_size < 2097152){
                if($image_ext == 'jpg' || $image_ext == 'png' || $image_ext == 'gif'){
                    if(move_uploaded_file($image_tmp,$image_path)){
                        $ins_sql = "INSERT INTO posts (title, description, image, category, status, date, author) VALUES ('$title', '$description', '$image_db_path', '$_POST[category]', '$_POST[status]','$date', '$_SESSION[user]')";
			...
			$ins_sql = "INSERT INTO posts (title, description, category, status, date, author) VALUES ('$title', '$description', '$_POST[category]', '$_POST[status]','$date', '$_SESSION[user]')";
			if(mysqli_query($conn,$ins_sql)){
				header('Location: post_list.php');
			}else {
